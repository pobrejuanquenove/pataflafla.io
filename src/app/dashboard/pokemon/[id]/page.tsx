import { Pokemon } from '@/appcomponents';
...
export async function generateStaticParams() {
  const static151Pokemon = Array.from({ length: 150 }).map((v, i) => i + 1);
  return static151Pokemon.map((id) => ({ id: id.toString() }));
}
...
export async function generateMetadata({ params }: { params: Params }):Promise<Metadata> {
  try {
    ...
    return {
      ...
      description: `Pokemon error page: ${error}`,
    };
  }
}
